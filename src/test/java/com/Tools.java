...
public class Tools {
  ...
  public static void print(Object result) throws Exception {
    // ********************ResultSet**************************
    ...
    if (result instanceof ResultSetMetaData) {
      ...
      for (int j = 0; j < metaData.getColumnCount(); j++) {
        ...
        row.add(dataType);
        if (((Integer)dataType).toString().length() > cell_widths[5]) cell_widths[5] = ((Integer) dataType).toString().length();
        row.add(typeName);
        ...
        row.add(columnSize);
        if (((Integer)columnSize).toString().length() > cell_widths[8]) cell_widths[8] = ((Integer)columnSize).toString().length();
        row.add(precisionSize);
        if (((Integer)precisionSize).toString().length() > cell_widths[9]) cell_widths[9] = ((Integer)precisionSize).toString().length();
        row.add(scaleSize);
        if (((Integer)scaleSize).toString().length() > cell_widths[10]) cell_widths[10] = ((Integer)scaleSize).toString().length();
        row.add(isNullable);
        if (((Boolean) isNullable).toString().length() > cell_widths[11]) cell_widths[11] = ((Boolean) isNullable).toString().length();
        datas.add(row);
      }
